...
@section('content')
    ...
    @if(! $course->isPublished())
        ...
        <div class="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            <strong>{{ $course->statusLabel() }}</strong> {{ __t('academy.course.admin_preview') }}
        </div>
    @endif
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 my-5">
        ...
        <div class="mt-3 flex flex-wrap items-center gap-2 text-sm text-slate-500">
            ...
            @if($course->final_quiz_enabled)
                <span aria-hidden="true">·</span>
                <span>{{ __t('academy.common.final_quiz') }}</span>
            @endif
        </div>
        ...
        <div class="mt-4 flex flex-wrap items-center gap-3">
            <div class="w-44 h-2 rounded-full bg-slate-100 overflow-hidden"
                 ...
                 aria-label="{{ __t('academy.common.progress_aria', ['done' => $done, 'total' => $total]) }}">
            ...
            <span class="text-sm text-slate-500">
                {{ __t('academy.common.lessons_done', ['done' => $done, 'total' => $total]) }}
                @if($leftLabel && $done < $total)
                    · {{ __t('academy.course.time_left', ['time' => $leftLabel]) }}
                @endif
            </span>
        </div>
    </div>
                        ...
                        @if($certificate)
                            <a href="{{ route('certificates.download', $certificate) }}"
                               class="inline-flex justify-center rounded-lg bg-ok text-white font-semibold px-6 py-3 hover:bg-green-700">
                                {{ __t('academy.common.download_certificate') }}
                            </a>
                        @endif
                        @if($nextCourse)
                        ...
                        @else
                            <a href="{{ route('academy.home') }}"
                               class="inline-flex justify-center rounded-lg border border-slate-300 text-slate-600 font-semibold px-6 py-3 hover:bg-slate-50">
                                {{ __t('academy.course.back_to_all') }}
                            </a>
                        @endif
                    </div>
                    ...
                    @auth
                        <div class="mt-5 pt-5 border-t border-slate-100">
                            @if(session('feedback_saved'))
                                <p role="status" class="text-sm font-semibold text-ok">{{ __t('academy.course.feedback_thanks') }}</p>
                            @endif

                            @if($feedback)
                                <p class="text-sm text-slate-500">
                                    {{ __t('academy.course.feedback_you_said') }}
                                    <strong class="text-navy">{{ $feedback->is_positive ? __t('academy.course.feedback_useful') : __t('academy.course.feedback_not_useful') }}</strong>.
                                </p>
                            @else
                                <p class="text-sm font-semibold text-navy mb-1">{{ __t('academy.course.feedback_question') }}</p>
                                <p class="text-sm text-slate-500 mb-3">{{ __t('academy.course.feedback_private') }}</p>
                            @endif

                            <form method="POST" action="{{ route('academy.course.feedback', $course) }}" class="mt-2">
                                @csrf
                                <label for="feedback-comment" class="vh">{{ __t('academy.course.feedback_comment') }}</label>
                                <textarea id="feedback-comment" name="comment" rows="2" maxlength="1000"
                                          placeholder="{{ __t('academy.course.feedback_placeholder') }}"
                                ...
                                <div class="mt-3 flex flex-wrap gap-3">
                                    <button type="submit" name="is_positive" value="1"
                                        ...
                                        <span aria-hidden="true">👍</span> {{ __t('academy.course.useful') }}
                                    </button>
                                    <button type="submit" name="is_positive" value="0"
                                        ...
                                        <span aria-hidden="true">👎</span> {{ __t('academy.course.not_useful') }}
                                    </button>
                                </div>
                            </form>
                        </div>
    ...
    @if($course->final_quiz_enabled)
        <div class="mt-6 bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <div class="flex items-start gap-4">
                ...
                <div class="min-w-0 flex-1">
                    <h2 class="text-xl font-extrabold text-navy">{{ __t('academy.course.final_heading') }}</h2>

                    @auth
                        @if($certificate)
                            <p class="text-slate-500 text-sm mt-1">{{ __t('academy.course.passed_with', ['percent' => $certificate->score_percent]) }}</p>
                            <div class="mt-4 flex flex-col sm:flex-row gap-3">
                                <a href="{{ route('certificates.download', $certificate) }}"
                                   class="inline-flex justify-center rounded-lg bg-ok text-white font-semibold px-6 py-3 hover:bg-green-700">
                                    {{ __t('academy.common.download_certificate') }}
                        ...
                        @else
                            ...
                            <button type="button" disabled
                                ...
                                🔒 {{ __t('academy.course.locked') }}
                            </button>
                        @endif
                    @else
                        <p class="text-slate-500 text-sm mt-1">{{ __t('academy.course.login_for_final') }}</p>
                        <a href="{{ route('login') }}"
                           class="mt-4 inline-flex w-full sm:w-auto justify-center rounded-lg bg-brand text-white font-semibold px-6 py-3 hover:bg-blue-700">
                            {{ __t('academy.course.login_to_continue') }}
                        </a>
                    @endauth
                </div>
            </div>
